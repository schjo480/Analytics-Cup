...
gb_fit <- gb_wf %>%
  finalize_workflow(select_best(gb_tune)) %>%
  fit(train_df)

# Variable Importance Plot
gb_fit %>% extract_fit_parsnip() %>% 
  vip::vip(geom='col', num_features = ncol(train_df))

# Apply on (internal) test set
test_predictions <- 
  predict(gb_fit, test_df)%>% 
  bind_cols(predict(gb_fit, test_df, type = "prob")) %>% 
  bind_cols(test_df %>% select(OFFER_STATUS))
...
sens_spec <- function(dt, score_column, labels_column, threshold){
  # Use confusion matrix
  cm <- confusion_matrix(as.data.table(dt), score_column, labels_column, threshold)
  ...
  sens <-  TP/(TP+FN)
  spec <-  TN/(FP+TN)
  return(data.table(sensitivity=sens, specificity=spec, bac = (sens + spec)/2, t=threshold))
}


thresholds2 <- seq(0.7, 0.9, 0.01)
bac_comparison <- rbindlist(lapply(thresholds2, function(t){sens_spec(test_predictions, ".pred_1", "OFFER_STATUS", t)}))
bac_comparison[bac == max(bac)]


external_prediction <- predict(gb_fit, df_test, type = "prob")
external_test_result <- data.table(id = df_test$TEST_SET_ID, prediction = as.numeric(external_prediction$.pred_1 > bac_comparison[bac == max(bac), t]))
write_csv(external_test_result, "predictions.csv")